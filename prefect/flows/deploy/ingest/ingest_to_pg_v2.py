...
def download_from_bucket(dataset_date) -> Path:
    """state directory target and source"""
    gcs_path = f"data/detik_scrap_{dataset_date}.parquet"
...
def ingest_to_pg():
    ...
    path = download_from_bucket(dataset_date)

    """2. transform"""
    data = read_data(path)

    """3. truncate insert to table"""
    insert_to_pg("detik_scrap_table", data)
